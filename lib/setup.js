var examples = {}
  , selector = 'canvas'
  , size = { width: innerWidth - 250, height: innerHeight * .9 }
...
function loadGist() {
  ...
  d3.json("https://api.github.com" + "/gists/" + id, function (err, data) {
    cleanup()
    files = data.files
    var injection = files['index.html'].content.replace('append("svg")', 'select("canvas").call(pathgl)')
...
//if its not public, reload when build script detects change
if (! location.href.match(/adnan/))
